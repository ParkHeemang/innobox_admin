...
function multi_update(sel)
{
    ...
    if (sel.value == 'del') {
        if (!confirm("선택한 이모티콘을 삭제합니다.\n\n비회원만 삭제됩니다.\n\n회원을 삭제하려면 회원관리 메뉴를 이용해주세요.\n\n실행하시겠습니까?"))
        {
            sel.selectedIndex = 0;
            return;
        }
    } else if (!confirm("선택한 이모티콘을 " + sel.options[sel.selectedIndex].innerHTML + "\n\n실행하시겠습니까?")) {
        sel.selectedIndex = 0;
        return;
    }
				...
				<div class="box">
					<div class="box-header">
						<nav class="navbar navbar-default navbtn" style="z-index: 1000000000000000000000000000000000">
							<div class="text-right">							
								<input type="submit" name="act_button" value="선택이동" onclick="document.pressed=this.value" class="btn btn-default btn_02">
								<input type="submit" name="act_button" value="선택삭제" onclick="document.pressed=this.value" class="btn btn-default btn_02">
								<a href="./form_write.php?page=<?php echo $page?>&amp;fg_no=<?php echo $fg_no?>" class="btn btn-danger">이모티콘 추가</a>			
							</div>
						</nav>
					</div>				
					<!-- /.box-header -->
					<div class="box-body">			
						...
						<div class="local_sch01 local_sch sms_preset_sch">
							...
							<select name="fg_no" onchange="location.href='<?php echo $_SERVER['SCRIPT_NAME']?>?fg_no='+this.value;" class="form-control">
							...
							<form name="search_form" method="get" action="<?php echo $_SERVER['SCRIPT_NAME']?>" class="form_list" class="form_list">
							...
							<input type="text" name="sv" value="<?php echo get_text($sv) ;?>" id="sv" required class="frm_input form-control required" >
						...
						<ul id="sms5_preset" class="sms5_box">
							...
							for($i=0;$res = sql_fetch_array($qry);$i++)
							{
								...
								$fo_file = '/sms/0/'.$res['fo_file'];
								$fo_size = @getimagesize(G5_DATA_PATH.$fo_file);
								if ($res['fo_file'] && file_exists(G5_DATA_PATH.$fo_file) && preg_match('%^image/(jpeg|gif|png)$%', $fo_size['mime'])) {
...
$(function() {
    ...
    $('.li_file').on('click', function() {
        $this = $(this),
        $view = $this.siblings('.li_image:last');
        $view.show();
    });
    // 이미지 미리보기 닫기
    $('.li_image').on('click', function() {
        $(this).hide();
    });
});
