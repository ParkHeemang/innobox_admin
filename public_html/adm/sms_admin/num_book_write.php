					...
					<div class="box-header">
						<nav class="navbar navbar-default navbtn" style="z-index: 1000000000000000000000000000000000">
							<div class="text-right">							
								<input type="submit" value="확인" class="btn_submit btn btn-primary" accesskey="s" onclick="return book_submit();">
								<a href="./num_book.php?<?php echo clean_query_string($_SERVER['QUERY_STRING']); ?>" class="btn btn_02 btn-default">목록</a>			
							</div>
						</nav>
					</div>
					<!-- /.box-header -->
					<div class="box-body">							
						...
						<div class="tbl_frm01 tbl_wrap">
							...
							<tr>
								...
								<td>
									<select name="bg_no" id="bg_no" required class="required class form-control">
							...
							<tr>
								...
								<td><input type="text" name="bk_name" id="bk_name" maxlength="50" value="<?php echo $write['bk_name']?>" required class="frm_input form-control required"></td>
							</tr>
							<tr>
								<th scope="row"><label for="bk_hp">휴대폰번호<strong class="sound_only"> 필수</strong></label></th>
								<td>
									<input type="text" name="bk_hp" id="bk_hp" value="<?php echo $write['bk_hp']?>" required class="frm_input form-control required">
        ...
        success: function(data) {
            if(data.error) {
                ...
                if( !list_text ){ // 중복 휴대폰 번호가 없다면 submit
                    if($check_msg.lentgh> 0)
                        $check_msg.remove();

//                    $("#exist_msg").text("<?php echo $exist_msg_2; ?>");
                    is_submit = true;
                } else {
                    if($check_msg.lentgh < 1)
