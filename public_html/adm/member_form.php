<?php
	$sub_menu = "200101";          // 메뉴코드   
	include_once('./_common.php'); // 공통함수
	include_once('./admin.head.php');
	auth_check($auth[$sub_menu], 'w');
if ($w == '')                                                                                                                  //회원등록할때
{
    $required_mb_id = 'required';																				//아이디를 요구사항으로 줌
...
else if ($w == 'u')																									// 회원 수정에서 넘어올때 (이게 내가 사용하는 기능인듯)
{
    $mb = get_member($mb_id);
    if (!$mb['mb_id'])
    ...
    $required_mb_id = 'readonly';																	// 아이디를 수정할 수 없게함
...
if (isset($mb['mb_certify'])) {
    ...
    if (preg_match("/-/", $mb['mb_certify'])) {
        sql_query(" ALTER  `{$g5['member_table']}` DROP `mb_certify` ", false);
    }
} else {
...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>
		회원추가<small>서브 타이틀</small>
		</h1>
		<ol class="breadcrumb">
		<li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
		<li><a href="#">회원관리</a></li>
		<li class="active">회원추가</li>
		</ol>
	</section>
				...
				<div class="box">
					...
					<div class="box-header">
						<nav class="navbar navbar-default navbtn">							
							<div class="text-right">
								<a href="./member_list.php?<?php echo $qstr ?>" class="btn btn-info btn-list">목록</a>
								<input type="submit" value="<?php echo $html_title?>" class="btn btn-danger btn_submit" accesskey='s'>
							</div>							
						</nav>
					</div>
					<!-- /.box-header -->
					...
					<div class="box-body">				
						<table id="example2" class="table table-bordered table-hover dataTable member-form" role="grid" aria-describedby="example2_info">						
							...
							<td>
								<input type="text" name="mb_id" value="<?php echo $mb['mb_id'] ?>" id="mb_id" <?php echo $required_mb_id ?> class="frm_input form-control  <?php echo $required_mb_id_class ?>" size="15"  maxlength="20">
								<!-- $['mb_id']  테이블에서 불러온 값-->
							</td>
							<th scope="row"><label for="mb_password">비밀번호<?php echo $sound_only ?></label></th>
							<td><input type="password" name="mb_password" id="mb_password" <?php echo $required_mb_password ?> class="frm_input form-control  <?php echo $required_mb_password ?>" size="15" maxlength="20"></td>
							</tr>
							<tr>
							<th scope="row"><label for="mb_name">이름(실명)<strong class="sound_only">필수</strong></label></th>
							<td><input type="text" name="mb_name" value="<?php echo $mb['mb_name'] ?>" id="mb_name" required class="required frm_input form-control " size="15"  maxlength="20"></td>
							<th scope="row"><label for="mb_nick">닉네임<strong class="sound_only">필수</strong></label></th>
							<td><input type="text" name="mb_nick" value="<?php echo $mb['mb_nick'] ?>" id="mb_nick" required class="required frm_input form-control " size="15"  maxlength="20"></td>
							</tr>
							<tr>
							<th scope="row"><label for="mb_level">회원 권한</label></th>
							<td><?php echo get_member_level_select('mb_level', 1, $member['mb_level'], $mb['mb_level'],'class= "form-control"') ?></td>
							<th scope="row">포인트</th>
							...
							<td><input type="text" name="mb_email" value="<?php echo $mb['mb_email'] ?>" id="mb_email" maxlength="100" required class="required frm_input form-control  email" size="30"></td>
							...
							<td colspan="3" class="td_addr_line">						
								...
								<input type="text" name="mb_zip" value="<?php echo $mb['mb_zip1'].$mb['mb_zip2']; ?>" id="mb_zip" class="frm_input form-control readonly" size="5" maxlength="6">
								<button type="button"  class="btn btn-primary" onclick="dmap('mb_zip', 'mb_addr1', 'mb_addr2', 'mb_addr3' )">주소 검색</button><br>
								<input type="text" name="mb_addr1" value="<?php echo $mb['mb_addr1'] ?>" id="mb_addr1" class="frm_input form-control  readonly" size="60">
								<label for="mb_addr1">기본주소</label><br>
								<input type="text" name="mb_addr2" value="<?php echo $mb['mb_addr2'] ?>" id="mb_addr2" class="frm_input form-control " size="60">
								<label for="mb_addr2">상세주소</label>
								<br>
								<input type="text" name="mb_addr3" value="<?php echo $mb['mb_addr3'] ?>" id="mb_addr3" class="frm_input form-control " size="60">
							...
							<td colspan="3" class="mb_icon_td">
								<div class ="img_constraint_manual">
							...
							<td colspan="3"><input type="text" name="mb_<?php echo $i ?>" value="<?php echo $mb['mb_'.$i] ?>" id="mb_<?php echo $i ?>" class="frm_input form-control surplus" size="30" maxlength="255"></td>
							</tr>
							<?php } ?>
