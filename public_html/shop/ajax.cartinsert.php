...
$return['message'] = '상품이 장바구니에 담겼습니다.\\n바로 확인 하시겠습니까?';
...
if (!$tmp_cart_id)
{
    ...
    $return['message'] = '더 이상 작업을 진행할 수 없습니다. \\n브라우저의 쿠키 허용을 사용하지 않음으로 설정한것 같습니다. 브라우저의 인터넷 옵션에서 쿠키 허용을 사용으로 설정해 주십시오. \\n그래도 진행이 되지 않는다면 쇼핑몰 운영자에게 문의 바랍니다.';
...
for($i=0; $i<$count; $i++) {
    ...
    if(!$it['it_id']) {
        ...
        $ct_error[$it_id][] = '상품정보가 존재하지 않습니다.';
        continue;
    }

    if($opt_count && $_POST['io_type'][$it_id][0] != 0) {
        //alert('상품의 선택옵션을 선택해 주십시오.');
        $ct_error[$it_id][] = '['.$it['it_name'].'] 상품의 선택옵션을 선택해 주십시오.';
        continue;
    }
    ...
    for($k=0; $k<$opt_count; $k++) {
        if ($_POST['ct_qty'][$it_id][$k] < 1) {
            ...
            $ct_error[$it_id][] = '['.$it['it_name'].'] 상품의 수량은 1 이상 입력해 주십시오.';
            continue;
        }
    }
    ...
    if($it['it_buy_min_qty'] || $it['it_buy_max_qty']) {
        ...
        if($it['it_buy_min_qty'] > 0 && $sum_qty < $it['it_buy_min_qty']) {
            ...
            $ct_error[$it_id][] = '['.$it['it_name'].'] 상품의 선택옵션 개수 총합 '.number_format($it['it_buy_min_qty']).'개 이상 주문해 주십시오.';
            continue;
        }
        ...
        if($it['it_buy_max_qty'] > 0 && $sum_qty > $it['it_buy_max_qty']) {
            ...
            $ct_error[$it_id][] = '['.$it['it_name'].'] 상품의 선택옵션 개수 총합 '.number_format($it['it_buy_max_qty']).'개 이하로 주문해 주십시오.';
            continue;
        }
    ...
    for($k=0; $k<$opt_count; $k++) {
        ...
        if($row2['ct_id']) {
            ...
            if ($tmp_ct_qty + $ct_qty > $tmp_it_stock_qty)
            {
                ...
                $ct_error[$it_id][] = "[".$it['it_name']."] 상품의 [".$io_value."] 옵션 재고수량이 부족합니다. \\n현재 재고수량 : " . number_format($tmp_it_stock_qty) . " 개";
                continue;
            }
