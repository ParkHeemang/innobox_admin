...
// 테마에 orderform.sub.php 있으면 include
if(defined('G5_THEME_SHOP_PATH')) {
    $theme_orderform_file = G5_THEME_SHOP_PATH.'/orderform.sub.php';
    if(is_file($theme_orderform_file)) {
        include_once($theme_orderform_file);
        return;
        unset($theme_orderform_file);
    }
}
...
<div id="sod_frm">
    ...
    <?php
        // 복합과세처리
        $row[0] = array('it_id' => '123456789', 'it_name' => '인터넷가입사이트', 'it_price' => 10000);
        $row[1] = array('it_id' => '987654321', 'it_name' => '테스트2', 'it_price' => 20000);

        $goods = get_text($row[0]['it_name']);
        //if($default['de_tax_flag_use']) {
            $comm_tax_mny = round(($tot_tax_mny + $send_cost) / 1.1);
            $comm_vat_mny = ($tot_tax_mny + $send_cost) - $comm_tax_mny;
        //}
        $tot_sell_price = $row[0]['it_price'];
        $tot_price = $tot_sell_price;
    ?>
    <table>
        <tr>
            <th>사이트명</th>
            <td><?php echo $row[0]['it_name']?></td>
        </tr>
        <tr>
            <th>가격</th>
            <td>
                <?php echo number_format($row[0]['it_price'])?>
                <input type="hidden" name="it_id" value="<?php echo $row[0]['it_id']; ?>">
                <input type="hidden" name="it_name" value="<?php echo get_text($row[0]['it_name']); ?>">
                <input type="hidden" name="it_price" value="<?php echo $row[0]['it_price']; ?>">
            </td>
        </tr>
    </table>
    <!-- } 주문상품 합계 끝 -->
    ...
    <section id="sod_frm_pay">
        ...
        //if ($default['de_bank_use']) {
            ...
            echo '<input type="hidden" name="od_bank_account" value="농협 351-2738-3653-01 정병주">';
            echo '농협 351-2738-3653-01 정병주';
