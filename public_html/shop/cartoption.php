...
<div id="sit_sel_option">
    <ul id="sit_opt_added">
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            if($row['io_price'] > 0)
                $io_price = '(+'.number_format(abs($row['io_price'])).'원)';
        ...
        <li class="sit_<?php echo $cls; ?>_list">
            ...
            <span class="sit_opt_subj"><?php echo $row['ct_option']; ?></span><span class="sit_opt_prc"><?php echo $io_price; ?></span>
            <div class="ct_qty">
                <label for="ct_qty_<?php echo $i; ?>" class="sound_only">수량</label>
                <input type="text" name="ct_qty[<?php echo $it['it_id']; ?>][]" value="<?php echo $row['ct_qty']; ?>" id="ct_qty_<?php echo $i; ?>" class="frm_input sit_qty_num" size="5"><button type="button" class="sit_qty_plus btn_frmline">증가</button><button type="button" class="sit_qty_minus btn_frmline">감소</button><button type="button" class="btn_frmline sit_qty_del">삭제</button>
            </div>
        </li>
        <?php
        }
