...
if (isset($wr_id) && $wr_id) {
    ...
    if ($bo_table == 'portfolio') {
        $img = " select * from g5_board_file where (1) 
        ...
        ";
        $res = sql_fetch($img);
        $tit = " select * from g5_write_portfolio where (1) and wr_id = '".$wr_id."'
        
        ";
        $res2 = sql_fetch($tit);
        if ($res['bf_file']) {
            $seo['image'] = G5_DATA_URL.'/file/'.$bo_table.'/'.$res['bf_file'].'';
        }
        $seo['url'] = G5_URL.'/bbs/board.php?bo_table='.$bo_table.'&wr_id='.$wr_id.'';
        $seo['title'] = $res2['wr_1'];
        $seo['description'] = $res2['wr_2'];
    }
