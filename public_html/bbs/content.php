...
$content_file = G5_DATA_PATH."/content/".$co['co_id'].".php";
$content_mobile_file = G5_DATA_PATH."/content/".$co['co_id']."_mobile.php";
if ($is_admin)
...
$content_cssfile = G5_DATA_PATH."/content/".$co['co_id'].".css";
$content_cssUrl = G5_DATA_URL."/content/".$co['co_id'].".css";
if(is_file($content_cssfile))
{
	add_stylesheet('<link rel="stylesheet" type="text/css" href="' . $content_cssUrl . '" />', 0);
}
...
if(is_file($skin_file)) {
    ...
    /**
        작성자 : sriver 15.06.03
        컨텐츠 파일이 존재 할 경우 출력 없으면 db 값 출력
    */
    $device = $_GET['device'];
    ...
    if($device=="mobile")
    {
        if(is_file($content_mobile_file))
        {
            include($content_mobile_file);
        }
        else
        {
            include($skin_file);
